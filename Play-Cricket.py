...
print('Computer has chosen a number, you need to guess it if the number is odd or even.')


# loops until user inputs proper number as expected
while True:
    # checks if input is int type
    try:
        user_toss = int(input("\nPress '1' for odd or '0' if even..."))
        # breaks loop if input is right
        if user_toss in range(0,2):
            break
        # or int type out of range, continue to ask
        print('Invalid Input!')
        continue

    # string type input, continue to ask
    except:
        print('Invalid Input!')
        continue
...
if user_won_toss:
    ...
    while True:
        # gives output a string type
        user_dec = input("Press '0' for bat or '1' for bowl...")
